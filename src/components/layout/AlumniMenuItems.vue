...
import AlumniMenuItem from './AlumniMenuItem.vue';
  ...
  <q-list v-bind="$attrs" padding>
    ...
    <template v-if="items && items.length > 0">
      <template v-for="(item, index) in items" :key="`parent-${index}`">
        ...
        <template v-if="item.children && item.children.length > 0">
          ...
          >
            <template v-if="!page.children">
              <AlumniMenuItem :dark-text="darkText" :light-text="lightText" :dense :item="page">
        ...
        <template v-else>
          <AlumniMenuItem :dark-text="darkText" :light-text="lightText" :dense :item="item">
            <template #end="{ item: menuItem }">
              <slot name="end" v-bind="{ menuItem }" />
            </template>
          </AlumniMenuItem>
